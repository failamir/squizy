...
class PlayContestView: UIViewController, UIScrollViewDelegate,GADFullScreenContentDelegate, FBRewardedVideoAdDelegate {
...
//        self.mainQuestionView.DesignViewWithShadow()
       ...
       /* let xPosition = timerView.center.x //.frame.origin.x //topView.center.x + 130 //
        let yPosition = timerView.center.y - 10 //.frame.origin.y //topView.center.y + 6 //
        ...
        timerView.layer.addSublayer(progressRing) //topView */
...
//    }
    ...
    func clearColor(views:UIView...){
        for view in views{
            view.isHidden = false
            view.backgroundColor = UIColor.white.withAlphaComponent(0.8) //Apps.BASIC_COLOR//
...
//            self.navigationController?.pushViewController(resultView, animated: true)
    ...
    var btnY = 0
      func SetButtonHeight(buttons:UIButton...){
          ...
          btnY = Int(self.secondChildView.frame.height + self.secondChildView.frame.origin.y + 100) //+ self.lifeLineView.frame.height 
...
//        reviewQues.append(ReQuestionWithE.init(id: ques.id, question: ques.question, optionA: ques.optionA, optionB: ques.optionB, optionC: ques.optionC, optionD: ques.optionD, optionE:"0", correctAns: ques.correctAns, image: ques.image, level: "0", note: ques.note, quesType: ques.quesType, userSelect: opt))
//    }
    ...
    func ShowAlertForNotEnoughCoins(requiredCoins:Int, lifelineName:String){
        self.timer.invalidate()
        let msgTxt = !UserDefaults.standard.bool(forKey: "adRemoved") ? "\(Apps.NEED_COIN_MSG1) \(requiredCoins) \(Apps.NEED_COIN_MSG2)" : "\(Apps.NEED_COIN_MSG1) \(requiredCoins) \(Apps.NEED_COIN_MSG2) \n \(Apps.NEED_COIN_MSG3)" //RemoveAds
